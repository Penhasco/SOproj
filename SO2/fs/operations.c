...
static int open_fhandles;
static int can_open_files;
...
int tfs_init() {
    ...
    if (pthread_cond_init(&destroy_cond, NULL) != 0)
        return -1;
...
int tfs_open(char const *name, int flags) {
    ...
    if (ret != -1 && open_fhandles < MAX_OPEN_FILES)
        open_fhandles++;
