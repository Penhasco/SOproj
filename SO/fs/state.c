...
void state_init() {
    ...
    pthread_mutex_init(&fhandle_lock, NULL);
}
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                for (int i = 0; i <= INODE_BLOCKS; i++) {
                    inode_table[inumber].data_blocks_addr[i] = b;
                }
                ...
                for (int i = 0; i <= INODE_BLOCKS; i++) {
                    inode_table[inumber].data_blocks_addr[i] = -1;
                }
...
int inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_size > 0) {
        for (int i = 0; i < INODE_BLOCKS; i++) {
            if (data_block_free(inode_table[inumber].data_blocks_addr[i]) == -1) {
                return -1;
            }
        }
        if (inode_table[inumber].data_blocks_addr[BLOCK_OF_BLOCKS] != -1) {
            void *block_of_blocks = data_block_get(inode_table[inumber].data_blocks_addr[BLOCK_OF_BLOCKS]);
        
            if (block_of_blocks == NULL) {
                return -1;
            }
            
            for(int i = 0; i < BLOCK_SIZE / sizeof(int); i +=(int)sizeof(int)) {
                if (data_block_free(*(int *)block_of_blocks + i)) {
                    return -1;
                }
            }
        }
...
// TODO: proteger tabela de diretorios
int add_dir_entry(int inumber, int sub_inumber, char const *sub_name) {
    ...
    dir_entry_t *dir_entry =
        (dir_entry_t *)data_block_get(inode_table[inumber].data_blocks_addr[DIRECTORY_BLOCK]);
    if (dir_entry == NULL) {
        return -1;
    }
...
int find_in_dir(int inumber, char const *sub_name) {
    ...
    /* Locates the block containing the directory's entries */
    dir_entry_t *dir_entry =
        (dir_entry_t *)data_block_get(inode_table[inumber].data_blocks_addr[ROOT_DIR_INUM]);
    if (dir_entry == NULL) {
        return -1;
    }
